...
import { notifyAlertStarted, notifyAlertEnded } from "@/utils/audioState";
...
export const usePrayerTimeAlerts = (
      ...
      prayers.forEach(prayer => {
        ...
        switch (prayer) {
          ...
          case "Isha":
            ...
            break;
          case "Jummah":
            prayerTime = jummahSettings?.jamat1 || detailedTimes.zuhr_jamat; // Use Jummah Settings, fallback to Zuhr Jamat time
            break;
        }
  ...
  const playAlertSound = (prayerName: string) => {
    // SECURITY: strictly prevent loud jamat alerts from firing on mobile/desktop devices
    if (!isTV) {
      console.log(`[usePrayerTimeAlerts] Not playing ${prayerName} alert out loud — device is not a TV.`);
      return;
    }
